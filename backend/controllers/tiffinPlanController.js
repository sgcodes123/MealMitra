...
const validatePlan = ({ title, description, mealType, duration, price }, partial = false) => {
    if (!partial || title !== undefined) {
        if (typeof title !== "string" || title.trim().length < 2 || title.trim().length > 100)
            return "Plan name must be between 2 and 100 characters";
    }
    ...
    if (!partial || duration !== undefined) {
        if (!durations.includes(duration))
            return "Duration must be Daily, Weekly, or Monthly";
    }
...
const createPlan = async (req, res) => {
    try {
        const { title, description, mealType, duration, price } = req.body;
...
const getAllPlans = async (req, res) => {
    try {
        ...
        res.status(500).json({ message: "Failed to fetch plans. Please try again." });
    }
};
...
const updatePlan = async (req, res) => {
    try {
        ...
        const plan = await TiffinPlan.findByIdAndUpdate(req.params.id, updates, {
            new: true,
            runValidators: true,
        });
        ...
        res.status(500).json({ message: "Failed to update plan. Please try again." });
    }
};
...
const deletePlan = async (req, res) => {
    try {
        ...
        res.status(500).json({ message: "Failed to delete plan. Please try again." });
    }
};
